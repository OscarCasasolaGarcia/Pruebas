...
if datosArboles is not None:
    ...
    opcionArbol1O2 = st.radio("Seleccione el tipo de árbol que desea utilizar: ", ("Árbol de decisión (Regresión)", "Árbol de decisión (Clasificación)"))
            ...
            if X.size > 0:
                ...
                try:
                    ...
                    * **criterion**. Indica la función que se utilizará para dividir los datos. Puede ser (ganancia de información) gini y entropy (Clasificación). Cuando el árbol es de regresión se usan funciones como el error cuadrado medio (MSE). """)
                    ...
                    PronosticoAD = DecisionTreeRegressor(max_depth=Max_depth, min_samples_split=Min_samples_split, min_samples_leaf=Min_samples_leaf, criterion=Criterio)
                    #PronosticoAD = DecisionTreeRegressor(max_depth=8, min_samples_split=4, min_samples_leaf=2)
                    
                    PronosticoAD.fit(X_train, Y_train)
                    #Se genera el pronóstico
                    Y_Pronostico = PronosticoAD.predict(X_test)
                    st.subheader('Y_test vs Y_Pronóstico')
                    ...
                    Arbol.format = 'svg'
                    Arbol.render('ArbolDecision')
                    with open("ArbolDecision.svg", "rb") as file:
                        btn = st.download_button(
                        label="Haz click aquí para descargar el árbol de decisión generado (extensión SVG)",
                        data=file,
                        file_name="ArbolDecision.svg",
                        mime="image/svg"
                        )
                    ...
                    st.subheader('Pronóstico de datos basado en el modelo establecido')
    ...
    if opcionArbol1O2 == "Árbol de decisión (Clasificación)":
        ...
        if opcionVisualizacionArbolD == "Aplicación del algoritmo":
            st.header('Definición de variables predictoras (X) y variable clase (Y)')
            datosArbolesDecision = datosArbolesDecision.replace({'M': 'Malignant', 'B': 'Benign'})
            MatrizCorr = datosArbolesDecision.corr(method='pearson')
            st.subheader("Recordando la matriz de correlaciones: ")
            st.dataframe(MatrizCorr)
            ...
            st.subheader('Variables Predictoras')
            # Seleccionar los datos que se quieren visualizar
            datos = st.multiselect("Selecciona las variables predictoras", datosDelPronostico)
            ...
            if X.size > 0:
                ...
                st.subheader('Variable Clase')
                # Variable Clase 
                variablePronostico = st.selectbox("Variable a clasificar", datosArbolesDecision.columns.drop(datos),index=1)
                Y = np.array(datosArbolesDecision[variablePronostico])
                with st.expander("Da click aquí para visualizar el dataframe con la variable clase que seleccionaste:"):
                    st.dataframe(Y)
                    ...
                    * **criterion**. Indica la función que se utilizará para dividir los datos. Puede ser (ganancia de información) gini y entropy (Clasificación). Cuando el árbol es de regresión se usan funciones como el error cuadrado medio (MSE). """)
                    ...
                    with st.expander("Da click aquí para clasificar los datos que gustes"):
                        ...
                        if st.checkbox("Dar clasificación: "):
                            if ClasificacionAD.predict([dato])[0] == "Malignant":
                                st.error("Con un algoritmo que tiene una exactitud del: "+str(round(ClasificacionAD.score(X_validation, Y_validation)*100,2))+"%, la clasificación para el paciente: "+str(sujetoN)+" fue de 0 (CERO), es decir, el diagnóstico fue MALIGNO.")
                            elif ClasificacionAD.predict([dato])[0] == "Benign":
                                st.success("Con un algoritmo que tiene una exactitud del: "+str(round(ClasificacionAD.score(X_validation, Y_validation)*100,2))+ "%, la clasificación para el paciente: "+str(sujetoN)+" fue de 1 (UNO), es decir, el diagnóstico fue BENIGNO.")
                            else:
